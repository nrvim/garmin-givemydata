...
DB_PATH = str(Path(__file__).parent.parent / "garmin.db")
...
def upsert_hrv(conn: sqlite3.Connection, record: dict) -> None:
    ...
    conn.execute(
        ...
        """,
        {
            ...
            "start_timestamp": record.get("startTimestampLocal")
            or record.get("startTimestampGMT"),
            "end_timestamp": record.get("endTimestampLocal")
            or record.get("endTimestampGMT"),
            "raw_json": json.dumps(record),
        },
    )
...
def upsert_spo2(conn: sqlite3.Connection, record: dict, cal_date: str = None) -> None:
    ...
    if not d:
        return
    conn.execute(
        "INSERT OR REPLACE INTO spo2 (calendar_date, avg_spo2, min_spo2, max_spo2, raw_json) "
        "VALUES (?, ?, ?, ?, ?)",
        (
            d,
...
def upsert_floors(conn: sqlite3.Connection, record: dict, cal_date: str = None) -> None:
    ...
    if not d:
        return
    conn.execute(
        "INSERT OR REPLACE INTO floors (calendar_date, ascended, descended, goal, raw_json) "
        "VALUES (?, ?, ?, ?, ?)",
        (
            d,
...
def upsert_hydration(conn: sqlite3.Connection, record: dict, cal_date: str = None) -> None:
    ...
    if not d:
        return
    conn.execute(
        "INSERT OR REPLACE INTO hydration (calendar_date, goal_ml, intake_ml, raw_json) "
        "VALUES (?, ?, ?, ?)",
        (
            d,
...
def upsert_activity_weather(conn: sqlite3.Connection, activity_id: int, data) -> None:
    ...
    conn.execute(
        ...
        (
            ...
            record.get("weatherTypeDTO", {}).get("desc") if isinstance(record.get("weatherTypeDTO"), dict) else record.get("weatherType"),
            json.dumps(data),
        ),
    )
...
def save_to_db(conn: sqlite3.Connection, endpoint_name: str, data, cal_date: str = None) -> int:
    ...
    try:
        if name == "daily_summary":
        ...
        elif name in ("weight_range", "weight_latest", "weight", "weight_range_rest",
                       "weight_first", "goal_weight"):
            for rec in records:
        ...
        elif name in ("daily_summaries_avg", "stats_averages",
                       "daily_summaries_count", "sync_timestamp",
            ...
            log.debug("save_to_db: no handler for endpoint '%s', skipping %d records",
