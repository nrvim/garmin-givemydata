...
def download_activity_files(
    ...
    if not activity_ids:
        ...
        conn.close()
        activity_ids = [
            (r["activity_id"], r["activity_name"], r["start_time_local"]) for r in rows
        ]
    else:
    ...
    try:
        ...
        client._page.goto(
            "https://connect.garmin.com/modern/", wait_until="domcontentloaded"
        )
        time.sleep(2)
        ...
        print(
            f"  Downloaded: {downloaded}, Skipped: {skipped}, Failed: {failed}, Total: {len(activity_ids)}"
        )
