...
- Check that Python 3.10+ and Chrome are installed
- Create a virtual environment and install dependencies
- Download the browser driver
- Ask for your Garmin email and password (saved locally in `.env`, never sent anywhere)
...
1. A Chrome window opens and logs into Garmin Connect
2. If you have MFA/2FA, enter the code in the **terminal** or the **browser window**
3. **Do not close the Chrome window** — the script uses it to fetch your data. It closes automatically when done.
4. The script downloads your data year by year — full history takes about **30 minutes**
5. After health data, FIT files are downloaded for every activity
6. Done:
   ```
   Database status:
     Daily summaries: 3650 days
     Date range: 2016-04-01 to 2026-03-29
     FIT files: 71
     Location: garmin-givemydata/garmin.db
   ```

**What happens on subsequent runs:**

```
Database has data through 2026-03-29 (3650 daily records)
Fetching 2 days: 2026-03-28 to 2026-03-30
Mode: incremental
...
Import complete:
  daily_summary: 2 records
  Total: 3 records

FIT files: all 71 already downloaded
```

The script detects your last sync date, fetches only new data, and downloads only new FIT files. Takes **seconds**.
...
└── fit/                   # Original FIT files (lossless device data)
    ├── 2026-03-03_22045765569_Nuremberg_Road_Cycling.zip
    ├── 2026-02-28_22017095980_Nuremberg_Running.zip
    └── ...
```
...
├── browser_profile_stealth/    # Chrome session (gitignored)
└── pyproject.toml
```
...
**"Login failed"**: Delete the `browser_profile_stealth/` directory and run again for a fresh session.
...
**403 or session errors**: Session may have expired. Delete `browser_profile_stealth/` and re-login.
